...
import {DeleteTable1Action, DeleteUserAction} from "@/context/Features/Actions.js";
...
import {EditTable1} from "@/components/Admin/Table1/EditTable1.jsx";

export const Table1Actions = ({ table1Id}) => {
    ...
    return (
        <>
            ...
            {open && <Add actionName={'Edit'} name={'Trainer'} setOpen={setOpen} open={open} addAction={(setOpen)=><EditTable1 setOpen={setOpen} Table1Id={table1Id}/>}/>}
                    ...
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                            onClick={async () => {
                                ...
                                try {
                                    const response = await ActionsApi.DeleteTable1(table1Id)
                                    toast.dismiss(deletingLoader)
                                    if (response?.status === 200 && response.data) {
                                        dispatch(
                                            DeleteTable1Action({id : table1Id})
                                        )
                                        toast.success('table deleted successfully')
                                    }else
                                    {
                                        throw new Error("Couldn't delete the trainer try again later")
                                    }
                                }catch (e)
                                {
