...
export const AddTable1 = ({Table1Id}) => {
    ...
    const onSubmit = async (values) => {
        ...
        const loading =  toast.loading("adding table in progress...")

        try {
            const response = await ActionsApi.AddTable1(values);
            if(response.status === 200 && response.data.table1)
            {
                dispatch(AddTable1Action({table1 : response.data.table1}));
